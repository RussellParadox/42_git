...
/*   Updated: 2023/06/17 15:23:39 by gdornic          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_scale(t_settings *settings, t_map *map)
{
	...
	proj_xymax = orthogonal_projection((t_int3D){map->max.x, map->max.y, 0, 0}, \
		map, settings->map_base);
	proj_apex = orthogonal_projection((t_int3D){map->apex.x, map->apex.y, \
		map->apex.z, 0}, map, settings->map_base);
	proj_abyss = orthogonal_projection((t_int3D){map->abyss.x, map->abyss.y, \
		map->abyss.z, 0}, map, settings->map_base);
	settings->scale = fmin((double)(settings->max.x - 2 * settings->border.x) / \
		(fabs(orthogonal_projection((t_int3D){map->max.x, 0, 0, 0}, map, \
		settings->map_base).x) + fabs(orthogonal_projection((t_int3D){0, \
		map->max.y, 0, 0}, map, settings->map_base).x)), (double) \
		(settings->max.y - 2 * settings->border.y) / (fmax(fabs(proj_xymax.y), \
		fabs(proj_abyss.y)) + fmax(fabs(orthogonal_projection(\
		(t_int3D){0, 0, 0, 0}, map, settings->map_base).y), \
		fabs(proj_apex.y))));
}
